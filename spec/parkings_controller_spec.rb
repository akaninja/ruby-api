...
describe 'ParkingsController' do
  ...
  context '#create' do
    it 'returns plate json' do
      ...
      expect(parking_json[:plate]).to eq 'aaa-4444'
      expect(parking_json[:entrance_date]).to eq Date.today.to_s
    end
  end
  ...
  context '#checkout' do
    it 'returns plate json and checkout date' do
      ...
      expect(parking_json[:plate]).to eq 'aaa-4444'
      expect(parking_json[:entrance_date]).to eq '2023-11-11'
      expect(parking_json[:departure_date].to_s).to eq Date.today.to_s
    end
  end
end
